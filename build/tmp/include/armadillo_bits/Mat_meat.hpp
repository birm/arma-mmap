...
Mat<eT>::~Mat()
  ...
  if(mem_state == 0)
    ...
    if(n_elem > arma_config::mat_prealloc)
      {
      memory::release( access::rw(mem), (n_elem*sizeof(eT)), (mem_state==4) );
      }
    }
...
Mat<eT>::init_cold()
  ...
  if(n_elem <= arma_config::mat_prealloc)
    ...
    bool mmap_happened;
    access::rw(mem) = memory::acquire<eT>(n_elem, &mmap_happened);
    if (mmap_happened==true) {access::rw(mem_state)=4;}
    }
  }
...
Mat<eT>::init_warm(uword in_n_rows, uword in_n_cols)
  ...
  // bool mmap_happened=false; // delete if nothing explodes
  ...
  if(old_n_elem == new_n_elem)
    ...
    if(t_mem_state == 4)
      {
      arma_extra_debug_print("Mat::init(): unmaping memory");
      //bool unmap_state=(int (munmap((void*)access::rw(mem), old_n_elem)) == -1); // removed because strange free.
      memory::release( access::rw(mem), (old_n_elem*sizeof(eT)), (mem_state==4) );
      //arma_debug_check(unmap_state,"arma::memory::release(): unable to unmap matrix");
      }
